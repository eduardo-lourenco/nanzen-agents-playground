...
The implementation should make data completeness explicit, move financial reconciliation
into deterministic code, and only report success after the required tools complete and the
expected PDF exists.
...
- The billing task identifies `reconcile_billing` and `create_report` as required tools.

### 4. Add Execution Postconditions and Their Tests

The runner currently treats any returned final answer as success. Add lightweight execution
postconditions without attempting to validate the report's financial contents yet.

Add successful-call state to the two required tools:

- `BillingSummaryTool` should record success only after reconciliation completes.
- `PDFReportTool` should record a created report only after the file is written successfully and
  confirmed to be non-empty.
- Invalid report input or failed PDF generation should raise an exception instead of returning
  an error-looking string that the agent can mistake for success.
- Prefer a structured successful report result:

  ```python
  {
      "ok": True,
      "path": ".../billing_summary_merid001.pdf",
      "size_bytes": 12345,
  }
  ```

Add optional task metadata to the billing task:

```python
"required_tools": ["reconcile_billing", "create_report"],
"expected_report": "billing_summary_merid001.pdf",
```

Pass this metadata through `src/challenge/runner.py` to `ActorAgent`. After the agent returns,
verify that:

- Every required tool recorded at least one successful invocation.
- The expected report was one of the files successfully created.
- The report exists inside `output/`.
- The report is non-empty.

Raise an exception when a postcondition fails so the runner reports `[FAIL]` instead of `[OK]`.
Include verified tool names and the artifact path in the task result or logs.

Add postcondition tests as part of this step. Verify that:

- A missing reconciliation call fails the task.
- A failed reconciliation call does not count as success.
- A missing report call fails the task.
- A report generation error does not count as success.
- A nonexistent or empty expected report fails the task.
- Successful required tool calls plus a non-empty expected report pass.

These checks confirm execution, not semantic correctness of the PDF.
...
Confirm that the runner reports verified success and identifies the generated artifact. Inspect
`output/billing_summary_merid001.pdf` manually and verify that it contains:
...
- `[OK]` requires successful reconciliation, successful report creation, and a non-empty expected
  PDF.
